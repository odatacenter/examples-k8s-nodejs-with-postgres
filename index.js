...
(async () => {
  if (!process.env.POSTGRES_HOST || !process.env.POSTGRES_USER || !process.env.POSTGRES_PASSWORD) {
    throw Error("Required environments not found");
  }
  const sequelize = new Sequelize(
    `postgres://${process.env.POSTGRES_USER}:${process.env.POSTGRES_PASSWORD}@${process.env.POSTGRES_HOST}/${process.env.POSTGRES_DB || "heroes"}`,
    {
  ...
  await server.register([
    ...
    Vision,
    {
      ...
      options: {
        info: {
          title: "Node.js hero app with postgres example",
          version: "1.0",
        },
      }
    },
